...
class AuthRepositoryAdapter implements IAuthRepository {
  ...
  Future<bool> hasAnyAdmin() async {
    ...
    final query = await (db.selectOnly(
      db.admins,
  ...
  Future<bool> login(String username, String password) async {
    ...
    final user =
        await (db.select(db.admins)..where(
              (tbl) =>
                  tbl.username.equals(username) &
                  tbl.passwordHash.equals(hashed),
            ))
            .getSingleOrNull();
    if (user != null) {
      await (db.update(db.admins)..where((tbl) => tbl.id.equals(user.id)))
          .write(AdminsCompanion(lastLoginAt: d.Value(DateTime.now())));
      return true;
    }
    return false;
  }
